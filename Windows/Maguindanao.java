...
proceedButton.addActionListener(new ActionListener() {
    ...
    private void performTSP() {
        ...
        resultTextArea.append("\t\t\tAll Possible Routes on Maguindanao" + "\n");
        ...
        //change of index to 3
        path[0] = 3; 
        visited[3] = true;
        tsp(3, 1, 0, path, graph, visited);
    ...
    private void tsp(int currentVertex, int count, int dist, int[] path, int[][] graph, boolean[] visited) {
        if (count == numVertices && graph[currentVertex][3] > 0) { // Check if all vertices are visited and there's a connection back to St. John
            String route = buildRouteString(path, dist + graph[currentVertex][3]); // Add the distance back to St. John
            resultTextArea.append(route + "\n");
            if (dist + graph[currentVertex][3] < shortestDistance) {
                shortestDistance = dist + graph[currentVertex][3]; // Add the distance back to St. John
                shortestPath = route;
            }
            return;
        }
    ...
    private String buildRouteString(int[] path, int dist) {
        ...
        sb.append(" --> ").append(locations[3]).append(" = ").append(dist).append("km");
        return sb.toString();
    }

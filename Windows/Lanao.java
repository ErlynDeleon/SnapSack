...
proceedButton.addActionListener(new ActionListener() {
    ...
    private void tsp(int currentVertex, int count, int dist, int[] path, int[][] graph, boolean[] visited) {
        if (count == numVertices && graph[currentVertex][2] > 0) { // Check if all vertices are visited and there's a connection back to St. John
            String route = buildRouteString(path, dist + graph[currentVertex][2]); // Add the distance back to St. John
            resultTextArea.append(route + "\n");
            if (dist + graph[currentVertex][2] < shortestDistance) {
                shortestDistance = dist + graph[currentVertex][2]; // Add the distance back to St. John
                shortestPath = route;
            }
            return;
        }
